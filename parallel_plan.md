...
4. **During A4/B4:** ✅ Completed. B4's `ScoringContext` accepts optional `ReplayResult` from A4's replay output. Both are wired into the verifier pipeline.
...
1. **Build LLM-based discovery (`agent/discovery/llm-discovery.ts`)** — send arbitrary TypeScript to Claude, get back `StateMachineIR`. This is what makes the tool work on any stateful code, not just reducers written in one exact pattern. Without this, the demo is limited to a narrow reducer shape.

Nice-to-have but not blocking:

2. **One compelling real-world example** — something more relatable than a counter (auth state, shopping cart, form wizard) to show during the demo.
3. **Broaden the IR beyond single-field state** — multi-field state machines will come naturally once LLM discovery is in place.
4. A6/B6 (CI caching, snapshot tests) — production hardening, not demo-critical.
